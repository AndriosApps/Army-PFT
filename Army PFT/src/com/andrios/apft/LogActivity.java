...
public class LogActivity extends Activity {
	...
	private void setConnections() {
		...
		logAdapter = new LogAdapter(this, R.layout.list_item_apft_entry,
				notesList);
	...
	private void setOnClickListeners() {
			...
			listView.setOnItemClickListener(new OnItemClickListener() {
				...
				public void onItemClick(AdapterView<?> arg0, View v, int row,
						long arg3) {
					Intent intent = null;
					if(notesList.get(row).getName().equals("APFT")){
						intent = new Intent(v.getContext(), ApftLogViewActivity.class);
					}else{
	...
	private void createLogDialog(){
		...
		alert.setItems(items, new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				if(which == 0){
					Intent intent = new Intent(LogActivity.this.getBaseContext(), ApftLogViewActivity.class);
					intent.putExtra("list", notesList);
